...
router.get('/', async (req, res) => {
  try {
    const { unit = '', since = '', limit = '50' } = req.query;
    const n = Math.min(Math.max(parseInt(limit) || 50, 1), 500);

    const args = ['-n', String(n), '--no-pager', '-o', 'json'];
    if (unit) {
      if (!validUnit.test(unit)) return res.status(400).json({ error: 'Invalid unit name' });
      args.push('-u', unit);
    }
    if (since) {
      if (!/^[\w\s:/-]+$/.test(since)) return res.status(400).json({ error: 'Invalid since value' });
      args.push('--since', since);
    }

    const out = await execAsync(`journalctl ${args.map(a => a.includes(' ') ? `"${a}"` : a).join(' ')}`);
    res.json(parseJournalOutput(out));
  } catch (err) {
